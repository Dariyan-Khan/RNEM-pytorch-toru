...
def change_data_in_pickle():
	with open('train_obs_10.pickle', 'rb') as file:
		...
		new_loaded_data = new_loaded_data.unsqueeze(-2).unsqueeze(-2)
		print(f"==>> new_loaded_data.shape: {new_loaded_data.shape}")
		file.close()
...
def create_h5_files():
	with open('torch_train_obs_10.pickle', 'rb') as file:
		...
		hf = h5py.File('torch_train_obs_10_last_channel.h5', 'w')
